...
<body class="bg-slate-50 text-neutral-800 min-h-screen pb-12">
    ...
    <header class="bg-white border-b border-neutral-200 py-4 px-6 mb-8">
        <div class="max-w-5xl mx-auto flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                {{-- KEMBALI --}}
                <a href="{{ route('home') }}" aria-label="Kembali ke halaman utama" title="Kembali"
                ...
                {{-- LOGO --}}
                <a href="{{ route('home') }}" class="font-extrabold text-xl tracking-wider text-neutral-800 uppercase">
                    TAKE AND GO
                </a>
            </div>
            ...
            <div class="relative">
                ...
                <button id="riwayatMenuButton" type="button" aria-label="Menu pengguna" aria-expanded="false"
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div class="bg-white rounded-2xl border border-amber-200 shadow-sm p-4 sm:p-5 mb-8">
            <div
                class="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-neutral-200 gap-y-4 sm:gap-y-0">

                {{-- Total Peminjaman --}}
                <div class="flex items-center justify-center gap-3 px-2">
                    <div
                        class="w-10 h-10 rounded-xl bg-cyan-100 flex items-center justify-center text-cyan-700 shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
        ...
        <div class="relative space-y-4">
            ...
            <div
                class="hidden md:block absolute left-[21.5%] top-6 bottom-6 w-0 border-l-2 border-dashed border-slate-200 z-0">
            </div>

            @forelse ($timeline as $step)
                <div
                    class="relative z-10 bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm hover:shadow-md transition">
                    <div class="grid grid-cols-1 md:grid-cols-12 items-center gap-4">
                        ...
                        <div class="md:col-span-3 flex items-center gap-3">
                            <div
                                class="w-9 h-9 rounded-full {{ $step['icon_bg'] }} flex items-center justify-center shrink-0">
                        ...
                        <div class="md:col-span-4">
                            ...
                            @if (!empty($step['can_timer']))
                                ...
                                <div class="mt-2 flex items-center gap-2" data-timer
                                    data-start="{{ $step['start_unix'] }}"
                                    data-deadline="{{ $step['deadline_unix'] }}"
                                    data-now="{{ $step['server_now_unix'] }}">
                                    <span
                                        class="timer-late-chip hidden px-2 py-0.5 rounded-full bg-rose-600 text-white text-[10px] font-bold">TERLAMBAT</span>
                                    <span
                                        class="timer-text {{ $step['is_late'] ? 'text-rose-600' : 'text-purple-600' }} font-bold text-sm tabular-nums">--:--:--</span>
                                    <span class="timer-fine text-[11px] text-rose-500 font-semibold">
                                        @if (!empty($step['is_late']))
                                            Denda sementara:
                                            Rp{{ number_format($step['temporary_fine'], 0, ',', '.') }}
                                        @endif
                                    </span>
                                </div>
                            @elseif (($step['raw_status'] ?? '') === 'menunggu')
                                {{-- Status Booking: timer belum aktif --}}
                                <p class="text-[11px] text-slate-400 font-medium mt-1.5">Timer akan muncul saat status
                                    berubah menjadi Dipinjam.</p>
                            @endif
                        ...
                        {{-- Barang & Kategori --}}
                        <div
                            class="md:col-span-3 md:text-right border-t md:border-t-0 md:border-l border-slate-100 pt-3 md:pt-0 md:pl-4">
                            <p class="font-bold text-slate-800 text-xs sm:text-sm">{{ $step['item'] }}</p>
                            <p class="text-xs text-slate-400 mt-1">Jumlah: {{ $step['jumlah'] }}</p>
                            <span
                                class="inline-block mt-1 px-3 py-0.5 rounded-full {{ $step['cat_bg'] }} text-[10px] font-semibold">
                                {{ $step['category'] }}
                            </span>
                        </div>
            ...
            @empty
                <div class="bg-white rounded-2xl border border-slate-200 p-6 text-center text-sm text-slate-400">Belum
                    ada riwayat peminjaman.</div>
            @endforelse
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const successModal = document.getElementById('successBorrowModal');
            ...
            if (successModal) {
                const playSuccessSound = function() {
                    try {
                        ...
                        gainNode.gain.exponentialRampToValueAtTime(0.13, audioContext.currentTime + 0.03);
                        ...
                        console.log('Suara notifikasi tidak bisa dijalankan di browser ini.', error);
                    }
                };
